...
def _date_sort_value(value: Any) -> Tuple[int, datetime]:
    """Sort valid support dates first, oldest to newest; blanks/unknowns last."""
    text = normalize_spaces(value)
...
def sort_and_compact_rows(rows: List[Dict[str, str]]) -> List[Dict[str, str]]:
    """
    Keep populated support rows together at the top and order them by date
    from oldest to newest. Empty rows remain below them.
    """
    populated = [
        dict(row)
        for row in rows
        if any(
            normalize_spaces(row.get(key, ""))
            for key in ("supportArea", "date", "update", "status")
        )
    ]
    empty_count = max(0, len(rows) - len(populated))
    populated.sort(key=lambda row: _date_sort_value(row.get("date", "")))
    ...
    for index in range(len(populated) + 1, len(populated) + empty_count + 1):
        compacted.append(
            {
                "row": str(index),
                "supportArea": "",
...
def option_for_field(
    ...
    for option in options:
        ...
        if normalize_key(candidate) == desired_key:
            # These support-area dropdowns are editable combo boxes. Returning the
            # clean display value (instead of the source option's leading space)
            # prevents newly populated rows from appearing blank in some viewers.
            return desired if kind == "area" else option

    # Some rows dynamically remove options already used elsewhere. The field is
    # editable, so retaining the clean reviewed value is valid.
    return desired
...
def full_widget_field_name(annotation: Any) -> str:
    """Build a fully-qualified AcroForm name such as Dropdown.List3."""
    parts: List[str] = []
    current = annotation
    visited: set[int] = set()
...
def page_field_names(page: Any) -> set[str]:
    ...
    for annotation_ref in page.get("/Annots", []) or []:
        ...
        name = full_widget_field_name(annotation)
        if name:
            names.add(name)
    return names


def set_populated_row_highlights(writer: PdfWriter, populated_rows: Iterable[int]) -> None:
    """
    Give every populated support-plan row the same yellow CMYK background used
    by the original completed rows.
    """
    highlighted_names: set[str] = set()
    for row in populated_rows:
        highlighted_names.update(
            {
                f"Dropdown.List{row}",
                f"Date{row}_af_date",
                f"Dropdown-update{row}",
                f"Dropdown outcomes met{row}",
            }
        )

    yellow = ArrayObject(
        [FloatObject(0), FloatObject(0), FloatObject(1), FloatObject(0)]
    )

    for page in writer.pages:
        for annotation_ref in page.get("/Annots", []) or []:
            annotation = annotation_ref.get_object()
            field_name = full_widget_field_name(annotation)
            if field_name not in highlighted_names:
                continue

            mk = annotation.get("/MK")
            if mk is None:
                mk = DictionaryObject()
                annotation[NameObject("/MK")] = mk
            elif hasattr(mk, "get_object"):
                mk = mk.get_object()

            mk[NameObject("/BG")] = yellow

            # Force the viewer/pypdf to rebuild the widget appearance using the
            # updated background and value.
            if "/AP" in annotation:
...
def apply_field_updates(
    ...
    set_populated_row_highlights(writer, populated_rows)
...
async def fill_pdf(
    ...
    try:
        ...
        merged_rows = sort_and_compact_rows(merged_rows)

        # Sorting can move multiple existing rows, so all populated rows need their
        # values and appearance refreshed.
        touched_rows = [
            index
        ...
        completed_pdf = apply_field_updates(reader, field_updates, touched_rows)
